...
uint64_t DEFAULT_CURR_INSTR = (uint64_t)-1;
// ------------------------ GLOBAL VARIABLES -----------------------
...
uint64_t get_curr_instr(uint64_t *context) { return (uint64_t)*(context + 34); }
...
void set_curr_instr(uint64_t *context, uint64_t tick) { *(context + 34) = (uint64_t)tick; }
...
void implement_sleep(uint64_t *context)
{
  ...
  if (get_curr_instr(context) == DEFAULT_CURR_INSTR)
    set_curr_instr(context, global_ctr);

  // En lugar de tiempo utilizaemos cantidad de instrucciones
  if (global_ctr < get_curr_instr(context) + time) {}
  else {
    set_pc(context,get_pc(context)+INSTRUCTIONSIZE);  
  }
}
...
void init_context(uint64_t *context, uint64_t *parent, uint64_t *vctxt)
{
  ...
  set_curr_instr(context, DEFAULT_CURR_INSTR);
