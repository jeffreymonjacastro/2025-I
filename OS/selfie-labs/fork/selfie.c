...
uint64_t lock_owner = (uint64_t)-1; //* Syscall Lock
...
//* Syscall Lock
uint64_t SYSCALL_LOCK = 20; 
uint64_t SYSCALL_UNLOCK = 6; 

//* Syscall fork
uint64_t SYSCALL_FORK = 2000; // Max 2048 
...
//* Syscall Lock
void add_to_waiting_list(uint64_t *context);
uint64_t *remove_context(uint64_t *context, uint64_t *from);
void use_context(uint64_t *context);
uint64_t find_context_lock(uint64_t *context, uint64_t* list);

//* Syscall fork
void copy_address_space(uint64_t *context, uint64_t *child_context);
void deep_copy(uint64_t *context, uint64_t *child_context);
...
// +----+-----------------+
// | 32 | id              | process id of this context
// | 33 | parent process  | context of parent process
...
uint64_t* get_parent_process(uint64_t* context) { return (uint64_t*) *(context + 33);}
...
void set_parent_process(uint64_t *context, uint64_t *parent_process) { *(context + 33) = (uint64_t)parent_process; }
...
//* Syscall lock
uint64_t *waiting_lock_contexts = (uint64_t *)0; // singly-linked list 
...
uint64_t LOCK_UNOWNED = (uint64_t)-1; //* Syscall Lock
...
void implement_exit(uint64_t *context)
{
  ...
  //TODO: Delete context from used_contexts
...
void print_contexts(){
  ...
  printf("(print_contexts): used_contexts: null -> ");
  current = used_contexts;
  ...
  printf("\n(print_contexts): waiting_lock_contexts: null -> ");
  current = waiting_lock_contexts;
  while (current != (uint64_t *)0){
    i = get_id(current);
    printf("%lu -> ", i);
    current = get_next_context(current);
...
void implement_lock(uint64_t *context) {
  uint64_t context_id;
  context_id = get_id(context);

  if (lock_owner == LOCK_UNOWNED){
    // lock no ha sido tomado 
    // proceso actual toma el lock
    // solo en este caso, avanzar el program counter

    lock_owner = context_id;
    set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
  } else if (lock_owner == context_id) {
    // proceso ya tiene el lock
    set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
  } else if (find_context_lock(context, waiting_lock_contexts) == (uint64_t)0){
    // si el lock fue tomado y el proceso actual no esta en la fila de espera:
    // añadir el proceso actual a waiting_queue
    // quitar proceso actual de used_contexts

    used_contexts = remove_context(context, used_contexts);
    add_to_waiting_list(context);
  }
}
...
void implement_unlock(uint64_t *context) {
  ...
  // 1. Verificar si el contexto actual es dueño del lock. Si es asi, soltar el lock
  if (lock_owner == context_id) {
    lock_owner = LOCK_UNOWNED;

    // 2. en caso exista otro contexto en espera (en waiting_queue), ejecutarlo
    if (waiting_lock_contexts != (uint64_t *)0){
      waiting_context = waiting_lock_contexts;
      use_context(waiting_context);
      lock_owner = get_id(waiting_context);
    }
  } 
...
void implement_fork(uint64_t* context){
  ...
  deep_copy(context, child_context);
...
void do_ecall()
{
  ...
  else
  {
    ...
    else if (*(registers + REG_A7) == SYSCALL_LOCK) {} //* Syscall lock
    else if (*(registers + REG_A7) == SYSCALL_UNLOCK) {}
    else if (*(registers + REG_A7) == SYSCALL_FORK) { //* Syscall fork
      write_register(REG_A0);
    }
    else 
    {
...
//* Syscall Lock
void add_to_waiting_list(uint64_t *context){
  // push front new context in waiting_lock_contexts
  set_next_context(context, waiting_lock_contexts);
  set_prev_context(context, (uint64_t *)0);
  waiting_lock_contexts = context;
}
...
void use_context(uint64_t *context){
  ...
  waiting_lock_contexts = get_next_context(context);
...
void copy_address_space(uint64_t *context, uint64_t *child_context){
  ...
  start = get_code_seg_start(context);
  size = get_code_seg_size(context);
  vaddr = start;

  while(vaddr < start + size) {
    value = load_virtual_memory(get_pt(context), vaddr);
    map_and_store(child_context, vaddr, value);
    
    vaddr = vaddr + WORDSIZE;
  }

  // Copiar segmento de datos
  start = get_data_seg_start(context);
  size = get_data_seg_size(context);
  vaddr = start;
...
void deep_copy(uint64_t *context, uint64_t *child_context){
  ...
  // Copiar el PC (2)
  set_pc(child_context, get_pc(context));

  // Referencia al padre (33)
  set_parent_process(child_context, context);
  
  // Copiar todos los atributos del PCB, del 5 al 31
  while (i < CONTEXTENTRIES - 2) {
    *(child_context + i) = *(context + i);
    i = i + 1;
  }
  ...
  while (i < NUMBEROFREGISTERS){
    *(get_regs(child_context) + i) = *(get_regs(context) + i);
    i = i + 1;
  }

  // Copiar el address space
  copy_address_space(context, child_context);
...
uint64_t handle_system_call(uint64_t *context)
{
  ...
  else if (a7 == SYSCALL_LOCK) //* Syscall Lock
    implement_lock(context);
  else if (a7 == SYSCALL_UNLOCK)
    implement_unlock(context);
  else if (a7 == SYSCALL_FORK) //* Syscall Fork
