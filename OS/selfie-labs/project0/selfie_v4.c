...
// semaphore
// +----+-----------------+
// |  0 | value           | current value of the semaphore
// |  1 | next_waiter     | pointer to the next waiter
// +----+-----------------+
...
void implement_exit(uint64_t *context)
{
  ...
  if (parent_context != (uint64_t *)0)
  {
    ...
    if (find_context_bool(parent_context, waiting_children_contexts) == (uint64_t)1)
    {
  ...
  else 
  {
    if (is_virtual_address_valid(wstatus, WORDSIZE)){
      if (is_data_stack_heap_address(context, wstatus)){
        map_and_store(context, wstatus, get_wstatus(context));
