...
uint64_t* LOCK_CONTEXT = (uint64_t *)0;
...
uint64_t DEFAULT_WSTATUS = (uint64_t)-1;
uint64_t DEFAULT_START_INSTR = (uint64_t)-1;
...
//bankers
void emit_allocate();
void implement_allocate(uint64_t *context);

void emit_claim();
void implement_claim(uint64_t *context);
...
uint64_t SYSCALL_SEM_INIT = 25;
uint64_t SYSCALL_SEM_WAIT = 26;
uint64_t SYSCALL_SEM_POST = 27;
uint64_t SYSCALL_LOCK = 31;
uint64_t SYSCALL_UNLOCK = 32;
uint64_t SYSCALL_TICK = 4;
uint64_t SYSCALL_SLEEP = 5;
uint64_t SYSCALL_ALLOCATE = 6;
uint64_t SYSCALL_CLAIM = 7;
uint64_t SYSCALL_REQUEST = 8;
...
uint64_t* add_to_waiting_list(uint64_t *context, uint64_t *waiting_contexts);

uint64_t* remove_context(uint64_t* context, uint64_t* from);

uint64_t find_context_bool(uint64_t *context, uint64_t* list);
...
// | 34 | children        | number of running children
// | 35 | wstatus         | status wait syscall
// | 36 | ptr_children    | pointer to children contexts
// | 34 | start_instr     | start instruction
...
uint64_t get_children(uint64_t *context) { return *(context + 34); }
uint64_t get_wstatus(uint64_t *context) { return *(context + 35); }
uint64_t* get_ptr_children(uint64_t *context) { return (uint64_t *)*(context + 36); }
uint64_t get_start_instr(uint64_t *context) { return (uint64_t)*(context + 37); }
...
void set_children(uint64_t *context, uint64_t children) { *(context + 34) = children; }
void set_wstatus(uint64_t *context, uint64_t wstatus) { *(context + 35) = wstatus; }
void set_ptr_children(uint64_t *context, uint64_t* children) { *(context + 36) = (uint64_t)children; }
void set_start_instr(uint64_t *context, uint64_t tick) { *(context + 37) = (uint64_t)tick; }
...
// +----+-----------------+
// |  0 | id              | semaphore_id
// |  1 | value           | current value of the semaphore
// |  2 | last_waiter     | pointer to the last waiting context
// |  3 | first_waiter    | pointer to the first waiting context
// |  4 | next_semaphore  | pointer to the next semaphore
// +----+-----------------+

uint64_t SEMENTRIES = 5;

uint64_t get_sem_id(uint64_t* semaphore) { return *(semaphore + 0);}
uint64_t get_sem_value(uint64_t* semaphore) { return *(semaphore + 1);}
uint64_t* get_last_waiter(uint64_t* semaphore) { return (uint64_t *) *(semaphore + 2);}
uint64_t* get_first_waiter(uint64_t* semaphore) { return (uint64_t *) *(semaphore + 3);}
uint64_t* get_next_semaphore(uint64_t* semaphore) { return (uint64_t *) *(semaphore + 4);}

void set_sem_id(uint64_t* semaphore, uint64_t id) { *(semaphore + 0) = id;}
void set_sem_value(uint64_t* semaphore, uint64_t value) { *(semaphore + 1) = value;}
void set_last_waiter(uint64_t* semaphore, uint64_t* context) { *(semaphore + 2) = (uint64_t)context;}
void set_first_waiter(uint64_t* semaphore, uint64_t* context) { *(semaphore + 3) = (uint64_t)context;}
void set_next_semaphore(uint64_t* semaphore, uint64_t* next_semaphore) { *(semaphore + 4) = (uint64_t)next_semaphore;}

// ------------------------ GLOBAL VARIABLES -----------------------

uint64_t* semaphore_list; // Similar to used_contexts
uint64_t sem_id = 0; // general autoincrement id for semaphores

// ------------------------  INITIALIZATION  -----------------------

uint64_t *allocate_semaphore();
uint64_t *new_semaphore();
void init_semaphore(uint64_t *semaphore, uint64_t value);
uint64_t *create_semaphore(uint64_t value);


uint64_t *allocate_semaphore()
{
  return smalloc(SEMENTRIES * sizeof(uint64_t));
}
...
uint64_t *new_semaphore()
{
  ...
  semaphore = allocate_semaphore();

  // Push front to semaphore_list
  set_next_semaphore(semaphore, semaphore_list);
...
void init_semaphore(uint64_t *semaphore, uint64_t value)
{
  set_sem_id(semaphore, sem_id);
  sem_id = sem_id + 1;

  set_sem_value(semaphore, value); 

  set_first_waiter(semaphore, (uint64_t *)0); 
  set_last_waiter(semaphore, (uint64_t *)0); 
}
...
uint64_t *create_semaphore(uint64_t value)
{
  ...
  init_semaphore(semaphore, value);
...
uint64_t* find_semaphore(uint64_t id)
{
  uint64_t *semaphore;
  semaphore = semaphore_list;

  while (semaphore != (uint64_t *)0)
  {
    if (get_sem_id(semaphore) == id)
      return semaphore;

    semaphore = get_next_semaphore(semaphore);
  }
...
uint64_t *resource;    // TOTAL RESOURCES
uint64_t *available;   // AVAILABLE RESOURCES
uint64_t *claim;       // TO BE FULLFILED
uint64_t *allocation;  // HAVE
uint64_t N_PROCESSES = 0;
uint64_t N_RESOURCES = 3;

// ------------------------- INITIALIZATION ------------------------

void init_resources() {
  resource = smalloc(N_RESOURCES * sizeof(uint64_t));

  *(resource) = 9; 
  *(resource + 1) = 3; 
  *(resource + 2) = 6; 

  available = smalloc(N_RESOURCES * sizeof(uint64_t));
...
void set_matrix_value(uint64_t* matrix, uint64_t i, uint64_t j, uint64_t value)
{
  *(matrix + i * N_RESOURCES + j) = value;
}

uint64_t get_matrix_value(uint64_t* matrix, uint64_t i, uint64_t j)
{
  return *(matrix + i * N_RESOURCES + j);
}

void set_list_value(uint64_t* list, uint64_t i, uint64_t value)
{
  *(list + i) = value;
}

uint64_t get_list_value(uint64_t* list, uint64_t i)
{
  return *(list + i);
}
...
void add_row_matrix()
{
  ...
  new_claim = smalloc(N_PROCESSES * N_RESOURCES * sizeof(uint64_t));
  zero_memory(new_claim, N_PROCESSES * N_RESOURCES * sizeof(uint64_t));

  new_allocation = smalloc(N_PROCESSES * N_RESOURCES * sizeof(uint64_t));
  zero_memory(new_allocation, N_PROCESSES * N_RESOURCES * sizeof(uint64_t));

  i = 0;
  while (i < N_PROCESSES - 1)
  {
    j = 0;
    while (j < N_RESOURCES)
    {
      set_matrix_value(new_claim, i, j, get_matrix_value(claim, i, j));
      set_matrix_value(new_allocation, i, j, get_matrix_value(allocation, i, j));
      j = j + 1;
    }
    i = i + 1;
  }
...
void print_list(uint64_t* list) {
  ...
  while (i < N_RESOURCES)
  {
    printf("%lu ", get_list_value(list, i));
    i = i + 1;
  }
  printf("\n\n");
}
...
void print_matrix(uint64_t* matrix)
{
  ...
  while (i < N_PROCESSES)
  {
    ...
    while (j < N_RESOURCES)
    {
      printf("%lu ", get_matrix_value(matrix, i, j));
      j = j + 1;
    }
    printf("\n");
    i = i + 1;
...
// state
// +----+-----------------+
// |  0 | available       | N_RESOURCES
// |  1 | allocation      | N_PROCESSES * N_RESOURCES
// +----+-----------------+

uint64_t *set_bankers_state() {
  uint64_t i;
  uint64_t j;
  uint64_t *state;
  
  state = smalloc((N_RESOURCES + N_PROCESSES * N_RESOURCES) * sizeof(uint64_t));

  i = 0;
  while (i < N_RESOURCES)
  {
    set_list_value(state + 0, i, get_list_value(available, i));
    i = i + 1;
  }
  ...
  while (i < N_PROCESSES) {
    j = 0;
    while (j < N_RESOURCES) {
      set_matrix_value(state + 1, i, j, get_matrix_value(allocation, i, j));
      j = j + 1;
    }
    i = i + 1;
  }
...
uint64_t safe(uint64_t* state) {
  ...
  uint64_t possible;
  uint64_t found;
  uint64_t* s_available;
  uint64_t* s_allocation;

  k = (uint64_t)-1;

  rest = (uint64_t*)smalloc(N_PROCESSES * sizeof(uint64_t));
  s_available = (uint64_t*)smalloc(N_RESOURCES * sizeof(uint64_t));
  s_allocation = (uint64_t*)smalloc(N_PROCESSES * N_RESOURCES * sizeof(uint64_t));

  i = 0;
  while (i < N_PROCESSES) {
    set_list_value(rest, i, 1);
    i = i + 1;
  }

  s_available = state;
  s_allocation = state + 1;
  ...
  while(possible) {
    ...
    i = 0;
    while (i < N_PROCESSES) {
      if (get_list_value(rest, i) != 0) {
        j = 0;
        while (j < N_RESOURCES) {
          need = get_matrix_value(claim, i, j) - get_matrix_value(s_allocation, i, j);

          if (need <= get_list_value(s_available, j)) {
            k = i;
          } else {
            k = (uint64_t)-1;
          }
    ...
    if (found) {
      j = 0;
      while (j < N_RESOURCES) {
        set_list_value(s_available, j, get_list_value(s_available, j) + get_matrix_value(s_allocation, k, j));
        set_matrix_value(s_allocation, k, j, 0);
        set_list_value(rest, k, 0); // remove process k from rest
        j = j + 1;
      }
    } else {
      possible = 0;
  ...
  return list_size(rest) == 0;
}

// --------------- RESOURCE ALLOCATION ALGORITHM -------------------

void resource_allocation(uint64_t i, uint64_t j, uint64_t value){
  uint64_t *bankers_state;

  if (get_matrix_value(allocation, i, j) + value > get_matrix_value(claim, i, j)) {
    printf("Error: Request exceeds claim for process %lu resource %lu\n", i, j);
  }
  else if (value > get_list_value(available, j)) {
    printf("Error: Not enough resources available for process %lu resource %lu\n", i, j);
  }
  else {
    bankers_state = set_bankers_state();
  }

  if (safe(bankers_state)) {
    set_matrix_value(allocation, i, j, get_matrix_value(allocation, i, j) + value);
    set_list_value(available, j, get_list_value(available, j) - value);
    printf("Resource allocation successful for process %lu resource %lu with value %lu\n", i, j, value);
  } else {
    printf("Error: Allocation would lead to unsafe state for process %lu resource %lu\n", i, j);
  }
}
...
uint64_t *waiting_children_contexts = (uint64_t *)0; // double-linked list of contexts waiting for children to finish
  ...
  if (IS64BITSYSTEM)
  {
    ...
    if (waddr % sizeof(uint64_t) == 0)
      ...
      *(memory + waddr / sizeof(uint64_t)) =
          left_shift(load_word(memory, waddr + SINGLEWORDSIZE, may_be_double_word), SINGLEWORDSIZEINBITS) + sign_shrink(word, SINGLEWORDSIZEINBITS);
    else
      // replace high word
      *(memory + waddr / sizeof(uint64_t)) =
          left_shift(word, SINGLEWORDSIZEINBITS) + load_word(memory, waddr - SINGLEWORDSIZE, may_be_double_word);
  }
  else
    *(memory + waddr / sizeof(uint64_t)) = word;
}
...
uint64_t *encode_elf_header()
{
  ...
  store_word(header, 0, 0, EI_MAG0 + left_shift(EI_MAG1, 8) + left_shift(EI_MAG2, 16) + left_shift(EI_MAG3, 24));
  store_word(header, 4, 0, EI_CLASS + left_shift(EI_DATA, 8) + left_shift(EI_VERSION, 16) + left_shift(EI_OSABI, 24));
  ...
  if (EI_CLASS == ELFCLASS64)
  {
    ...
    store_word(header, 48, 1, e_flags + left_shift(e_ehsize, 32) + left_shift(e_phentsize, 48));
    store_word(header, 56, 1, e_phnum + left_shift(e_shentsize, 16) + left_shift(e_shnum, 32) + left_shift(e_shstrndx, 48));
  }
  else
  {
...
void implement_exit(uint64_t *context)
{
  ...
  i = 0;
  while (i < N_RESOURCES){
    set_list_value(available, i, get_list_value(available, i) + get_matrix_value(allocation, get_id(context), i));
    set_matrix_value(allocation, get_id(context), i, 0);
    i = i + 1;
  }
  ...
  if (parent_context != (uint64_t *)0)
  {
    ...
    set_wstatus(parent_context, get_exit_code(context) * 256);

    waiting_children_contexts = remove_context(parent_context, waiting_children_contexts);
...
void print_context(uint64_t *context){
  ...
  while (curr != (uint64_t *)0) {
    if (get_ptr_parent(curr) != (uint64_t *)0)
      printf("| %lu ", get_id(get_ptr_parent(curr)));
    else
      printf("| null ");

    printf("| %lu | %lu | -> ", get_id(curr), get_children(curr));
...
void implement_fork(uint64_t *context){
  ...
  *(parent_regs+REG_A0) = get_id(child_c); // return for parent is child's id
  ...
  // Set ptr to last child
  set_ptr_children(context, child_c);
}
...
{
  uint64_t status;
  status = *(get_regs(context)+REG_A0);

  // No children context has exited yet
  if (get_wstatus(context) == DEFAULT_WSTATUS) 
  {
    // Parent has childrens
    if (get_children(context) > (uint64_t)0) 
    {
      used_contexts = remove_context(context, used_contexts);
      waiting_children_contexts = add_to_waiting_list(context, waiting_children_contexts);
    }
    else
    {
  ...
  else 
  {
    if (status == (uint64_t)0){
      *(get_regs(context)+REG_A0) = get_id(get_ptr_children(context));
      set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
      return;
    }

    if (is_virtual_address_valid(status, WORDSIZE))
    {
      if (is_data_stack_heap_address(context, status))
      {
        map_and_store(context, status, get_wstatus(context));
        set_wstatus(context, DEFAULT_WSTATUS);
...
{
  uint64_t time;
  time = *(get_regs(context)+REG_A0);

  if (get_start_instr(context) == DEFAULT_START_INSTR)
    set_start_instr(context, global_ctr);

  // Instead of time, use number of instructions
  if (global_ctr > get_start_instr(context) + time) {
    set_pc(context,get_pc(context)+INSTRUCTIONSIZE);  
  }
}
...
void emit_sem_init()
{
  ...
  emit_load(REG_A0, REG_SP, 0); // *sem
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_load(REG_A1, REG_SP, 0); // value
  emit_addi(REG_SP, REG_SP, WORDSIZE);
...
void implement_sem_init(uint64_t *context)
{
  ...
  value = *(get_regs(context) + REG_A1);

  sem = create_semaphore(value);

  map_and_store(context, address, get_sem_id(sem));
...
void implement_sem_wait(uint64_t *context)
{
  ...
  id = load_virtual_memory(get_pt(context), address);

  sem = find_semaphore(id);

  if (get_sem_value(sem) == 0){
    used_contexts = remove_context(context, used_contexts);

    if (get_first_waiter(sem) == (uint64_t *)0){
      set_first_waiter(sem, context);
      set_last_waiter(sem, context);
    } else {
      set_next_context(get_last_waiter(sem), context); // Push back
      set_last_waiter(sem, context);
    }
...
void emit_sem_post()
{
  ...
  emit_addi(REG_A7, REG_ZR, SYSCALL_SEM_POST);
...
void implement_sem_post(uint64_t *context)
{
  ...
  if (get_sem_value(sem) == 0) {
    waiting_context = get_first_waiter(sem);

    if (waiting_context != (uint64_t *)0){
      set_first_waiter(sem, get_next_context(waiting_context));

      if (get_first_waiter(sem) == (uint64_t *)0)
        set_last_waiter(sem, (uint64_t *)0); // no more waiters

      use_context(waiting_context);
    }
  } 
...
void emit_allocate()
{
  create_symbol_table_entry(GLOBAL_TABLE, string_copy("allocate"),
                            0, PROCEDURE, VOID_T, 2, code_size);

  emit_load(REG_A0, REG_SP, 0); // Resource
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_load(REG_A1, REG_SP, 0); // Value
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_addi(REG_A7, REG_ZR, SYSCALL_ALLOCATE);
...
void implement_allocate(uint64_t *context)
{
  ...
  resource = *(get_regs(context) + REG_A0);
  value = *(get_regs(context) + REG_A1);

  set_matrix_value(allocation, get_id(context), resource, value);

  set_list_value(available, resource, get_list_value(available, resource) - value);
...
void emit_claim()
{
  create_symbol_table_entry(GLOBAL_TABLE, string_copy("claim"),
                            0, PROCEDURE, VOID_T, 2, code_size);
  ...
  emit_addi(REG_A7, REG_ZR, SYSCALL_CLAIM);
...
void implement_claim(uint64_t *context)
{
  ...
  set_matrix_value(claim, get_id(context), resource, value);
...
void emit_request()
{
  create_symbol_table_entry(GLOBAL_TABLE, string_copy("request"),
                            0, PROCEDURE, VOID_T, 2, code_size);
...
void implement_request(uint64_t *context)
{
  ...
  // Bankers
  resource_allocation(get_id(context), resource, value);
...
uint64_t print_code_context_for_instruction(uint64_t address)
{
  ...
  if (run)
  {
    w = printf_or_write(sprintf(string_buffer, "%s: pc==0x%lX", binary_name, address)) + print_code_line_number_for_instruction(address, code_start);
    if (symbolic)
      // skip further output
      return w;
    else
    ...
    return dprintf(output_fd, "0x%lX", address) + print_code_line_number_for_instruction(address, code_start) + dprintf(output_fd, ": ");
  else if (disassemble_verbose)
    return dprintf(output_fd, "0x%lX", address) + print_code_line_number_for_instruction(address, 0) + dprintf(output_fd, ": 0x%08lX: ", (uint64_t)ir);
  else
    return 0;
}
...
uint64_t print_add_sub_mul_divu_remu_sltu()
{
  return print_code_context_for_instruction(pc) + printf_or_write(sprintf(string_buffer, "%s %s,%s,%s",
...
uint64_t print_load()
{
  return print_code_context_for_instruction(pc) + printf_or_write(sprintf(string_buffer, "%s %s,%ld(%s)",
...
uint64_t print_beq()
{
  ...
  w = print_code_context_for_instruction(pc) + printf_or_write(sprintf(string_buffer, "%s %s,%s,%ld",
...
uint64_t print_jal()
{
  ...
  w = print_code_context_for_instruction(pc) + printf_or_write(sprintf(string_buffer, "%s %s,%ld",
...
uint64_t print_ecall()
{
  return print_code_context_for_instruction(pc) + printf_or_write(sprintf(string_buffer, "%s", get_mnemonic(is)));
}
...
void do_ecall()
{
  ...
  else
  {
    ...
    else if (*(registers + REG_A7) == SYSCALL_SEM_INIT){
      read_register(REG_A1);
    }
    else if (*(registers + REG_A7) == SYSCALL_SEM_WAIT){}
    else if (*(registers + REG_A7) == SYSCALL_SEM_POST){}
    else if (*(registers + REG_A7) == SYSCALL_LOCK){}
    else if (*(registers + REG_A7) == SYSCALL_UNLOCK){}
    else if (*(registers + REG_A7) == SYSCALL_SLEEP){} 
    else if (*(registers + REG_A7) == SYSCALL_ALLOCATE){
      read_register(REG_A1);
    } 
    else if (*(registers + REG_A7) == SYSCALL_CLAIM){
      read_register(REG_A1);
    }
...
uint64_t print_data_context()
{
  return dprintf(output_fd, "0x%lX", pc) + print_data_line_number() + dprintf(output_fd, ": ");
}
...
void selfie_disassemble(uint64_t verbose)
{
  ...
  while (pc < code_size)
  {
    ...
    number_of_written_characters = number_of_written_characters + print_instruction() + dprintf(output_fd, "\n");
  ...
  {
    number_of_written_characters = number_of_written_characters + print_data() + dprintf(output_fd, "\n");
...
void decode()
{
  ...
  { // could be ADD, SUB, MUL, DIVU, REMU, SLTU
    decode_r_format();
...
uint64_t* add_to_waiting_list(uint64_t *context, uint64_t *waiting_contexts)
{
  ...
  if (waiting_contexts != (uint64_t*)0)
    set_prev_context(waiting_contexts, context);

  waiting_contexts = context;

  return waiting_contexts;
}
...
uint64_t *new_context()
{
  ...
  // Push back to used_contexts
  // set_prev_context(context, used_contexts);
  // set_next_context(context, (uint64_t *)0);
  
  // if (used_contexts == (uint64_t *)0)
  //   used_contexts = context;
  // else if (used_contexts != (uint64_t *)0)
  //   set_next_context(used_contexts, context);
...
void init_context(uint64_t *context, uint64_t *parent, uint64_t *vctxt)
{
  ...
  set_ptr_children(context, (uint64_t *)0);
...
uint64_t handle_system_call(uint64_t *context)
{
  ...
  else if (a7 == SYSCALL_ALLOCATE)
    implement_allocate(context);
  else if (a7 == SYSCALL_CLAIM)
    implement_claim(context);
