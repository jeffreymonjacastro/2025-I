...
uint64_t SYSCALL_PTHREAD_CREATE = 6;
uint64_t SYSCALL_PTHREAD_JOIN = 7;
uint64_t SYSCALL_PTHREAD_EXIT = 8;
...
// | 36 | ptr_children    | pointer to last children context
// | 37 | start_instr     | start instruction
// | 38 | next_child      | pointer to next child context
// +----+-----------------+
...
uint64_t* get_next_child(uint64_t *context) { return (uint64_t *)*(context + 38); }
...
void set_next_child(uint64_t *context, uint64_t* next_child) { *(context + 38) = (uint64_t)next_child; }
...
void implement_exit(uint64_t *context)
{
  ...
  if (parent_context != (uint64_t *)0)
  {
    ...
    if (find_context_bool(parent_context, waiting_children_contexts))
    {
...
void implement_sem_wait(uint64_t *context)
{
  ...
  if (get_sem_value(sem) == 0){
    ...
    if (get_first_waiter(sem) == (uint64_t *)0){
      ...
      set_next_context(get_last_waiter(sem), context); 
      set_last_waiter(sem, context);
    }

  } else {
    set_sem_value(sem, get_sem_value(sem) - 1);
    // printf("(sem_wait): sem %lu decremented to %lu\n", get_sem_id(sem), get_sem_value(sem));
    set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
  } 
}
...
void implement_sem_post(uint64_t *context)
{
  ...
  if (get_sem_value(sem) == 0) {
    ...
    if (waiting_context != (uint64_t *)0){
      ...
      if (get_first_waiter(sem) == (uint64_t *)0)
        set_last_waiter(sem, (uint64_t *)0); 
  ...
  set_sem_value(sem, get_sem_value(sem) + 1);
  // printf("(sem_post): sem %lu incremented to %lu\n", get_sem_id(sem), get_sem_value(sem));
  set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
}
...
void implement_pthread_create(uint64_t* context)
{
  ...
  // SHARED: code, data segment and heap 
  set_lowest_lo_page(child_c, get_lowest_lo_page(context));
  set_highest_lo_page(child_c, get_highest_lo_page(context));
  ...
  *(parent_regs+REG_A0) = get_id(child_c); // return for parent is child's id
  ...
  // Set parent to last child
  set_ptr_parent(child_c, context);

  // push_front children
  if (get_ptr_children(context) == (uint64_t *)0) {
    set_ptr_children(context, child_c);
    set_next_child(child_c, (uint64_t*)0);
  } else {
    set_next_child(child_c, get_ptr_children(context));
    set_ptr_children(context, child_c);
  }
}
...
void implement_pthread_join(uint64_t* context)
{
  ...
  if (get_wstatus(context) == (uint64_t)0) 
  {
    if (get_children(context) > (uint64_t)0) 
    {
      used_contexts = remove_context(context, used_contexts);
  ...
  else 
  {
    ...
    if (is_virtual_address_valid(status, WORDSIZE))
    {
      if (is_data_stack_heap_address(context, status))
      {
        child_context = get_ptr_children(context);

        map_and_store(context, status, get_exit_code(child_context));
        set_wstatus(context, get_wstatus(context) - (uint64_t)1);

        *(get_regs(context)+REG_A0) = get_id(child_context);

        set_ptr_children(context, get_next_child(child_context));
...
void emit_pthread_exit()
{
  ...
  emit_load(REG_A0, REG_SP, 0); // return value
  emit_addi(REG_SP, REG_SP, WORDSIZE);
...
void implement_pthread_exit(uint64_t* context)
{
  ...
  {
    set_children(parent_context, get_children(parent_context) - (uint64_t)1);
    set_wstatus(parent_context, get_wstatus(parent_context) + (uint64_t)1);
...
void synchronize_try_brk(uint64_t new_program_break) {
  uint64_t* curr_context;

  curr_context = used_contexts;

  while(curr_context != (uint64_t*)0) 
  {
    set_program_break(curr_context, new_program_break);
    curr_context = get_next_context(curr_context);
  }

  curr_context = waiting_children_contexts;
...
uint64_t try_brk(uint64_t *context, uint64_t new_program_break)
{
  ...
  if (is_virtual_address_valid(new_program_break, WORDSIZE))
    if (is_address_between_stack_and_heap(context, new_program_break))
    {
      ...
      synchronize_try_brk(new_program_break);
...
void synchronize_brk()
{
  ...
  while (curr_context != (uint64_t *)0)
  {
    set_lc_malloc(curr_context, get_lc_malloc(curr_context) + 1);
...
void synchronize_store(uint64_t vaddr)
{
  uint64_t* curr_pt;
  uint64_t* curr_context;
  uint64_t should_synchronize;
  
  should_synchronize = 0;
  
  if (is_data_address(current_context, vaddr)) 
    should_synchronize = 1;
    
  if (is_heap_address(current_context, vaddr)) 
    should_synchronize = 1;
    
  if (is_code_address(current_context, vaddr)) 
    should_synchronize = 1;
  
  if (should_synchronize == 1)
  {
    curr_context = used_contexts;

    while (curr_context != (uint64_t *)0)
    {
      curr_pt = get_pt(curr_context);
      
      // semantics of store (double) word
      if (load_virtual_memory(curr_pt, vaddr) != *(registers + rs2))
        store_cached_virtual_memory(curr_pt, vaddr, *(registers + rs2));
      else
      {
  ...
  {
    // Private segment (stack) - only update current context
...
void do_ecall()
{
  ...
  else
  {
    ...
    else if (*(registers + REG_A7) == SYSCALL_PTHREAD_CREATE){
      write_register(REG_A0);
    }
    else if (*(registers + REG_A7) == SYSCALL_PTHREAD_JOIN){
      write_register(REG_A0);
    }
    else if (*(registers + REG_A7) == SYSCALL_PTHREAD_EXIT){}
    else
    {
...
void init_context(uint64_t *context, uint64_t *parent, uint64_t *vctxt)
{
  ...
  //* Threads
  set_next_child(context, (uint64_t *)0);
  set_wstatus(context, (uint64_t)0);
...
void synchronize_handle_page_fault(uint64_t page)
{
  uint64_t* curr_context;
  ...
  while(curr_context != (uint64_t*)0) 
  {
    map_page(curr_context, page, (uint64_t)palloc());

    if (is_heap_address(curr_context, get_virtual_address_of_page_start(page)))
      set_mc_mapped_heap(curr_context, get_mc_mapped_heap(curr_context) + PAGESIZE);
...
uint64_t handle_page_fault(uint64_t *context)
{
  ...
  if (pavailable())
  {
    synchronize_handle_page_fault(page);
