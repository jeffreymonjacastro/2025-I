...
void swap(uint64_t *c1, uint64_t *c2);
void sort_contexts();
...
void emit_priority_set()
{
  ...
  emit_load(REG_A0, REG_SP, 0); // PRIORITY
...
//     printf("null ->");
//     printf(" | %lu | %lu | ->", get_id(curr), get_priority(curr));
...
void swap(uint64_t *c1, uint64_t *c2)
{
  ...
  prev_c1 = get_prev_context(c1);
  next_c1 = get_next_context(c1);
  prev_c2 = get_prev_context(c2);
  next_c2 = get_next_context(c2);

  if (prev_c1 != (uint64_t *)0)
    set_next_context(prev_c1, c2);
  if (next_c1 != (uint64_t *)0)
    set_prev_context(next_c1, c2);
  if (prev_c2 != (uint64_t *)0)
    set_next_context(prev_c2, c1);
  if (next_c2 != (uint64_t *)0)
    set_prev_context(next_c2, c1);

  set_prev_context(c1, prev_c2);
  set_next_context(c1, next_c2);
  set_prev_context(c2, prev_c1);
  set_next_context(c2, next_c1);
}
...
void sort_contexts()
{
  ...
  curr_i = used_contexts;

  while (curr_i != (uint64_t *)0) {
    current_max = curr_i;
    curr_j = curr_i;

    while(curr_j != (uint64_t *)0) {
      if (get_priority(curr_j) > get_priority(current_max)) {
        current_max = curr_j;
      }
      curr_j = get_next_context(curr_j);
    }

    if (current_max != curr_i) {
      swap(curr_i, current_max);
    }

    curr_i = get_next_context(curr_i);
  }
}
...
void implement_lock(uint64_t *context)
{
  if (LOCK_OWNER == DEFAULT_LOCK_OWNER)
  {
    ...
    // Priority donation
    if (get_priority(context) > get_priority(LOCK_CONTEXT)) {
      set_prev_priority(LOCK_CONTEXT, get_priority(LOCK_CONTEXT));
      set_priority(LOCK_CONTEXT, get_priority(context));
      sort_contexts();
    }
  }
}
...
uint64_t mipster(uint64_t *to_context)
{
  ...
  while (1)
  {
    ...
    // * Syscall priority
    max_priority = get_priority(used_contexts);
    ...
    else
    {
      ...
      { 
        if (to_context != (uint64_t *)0) {
          if (get_priority(to_context) < max_priority) 
            to_context = used_contexts;
        } else 
          to_context = used_contexts;
      }
