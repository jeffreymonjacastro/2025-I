...
uint64_t children(uint64_t *context) { return (uint64_t)(context + 34); }
...
uint64_t get_children(uint64_t *context) { return (uint64_t *) *(context + 34); }
...
void set_children(uint64_t *context, uint64_t num) { *(context + 34) = num; }
...
void do_ecall()
{
  ...
  else
  {
    ...
    if (*(registers + REG_A7) == SYSCALL_ID) {
      ...
      write_register(REG_A0);
    } else if (*(register + REG_A7) == SYSCALL_WAIT) {
