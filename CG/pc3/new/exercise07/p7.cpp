...
Vector<ld> translate_point( Vector<ld> director, Vector<ld> to_translate){
   return to_translate + director;
}
...
void write_off(Vector<ld> director, string full_path_input_mesh, string full_path_output_mesh ){
    ...
    for (int i = 0; i < vertices; i++) {
        ...
        auto rotated = translate_point(director, original_point);
...
void write_ply( Vector<ld> director, string full_path_input_mesh, string full_path_output_mesh ){
...
string full_path_output_mesh){
   ...
   if(is_ply){
       write_ply( director,  full_path_input_mesh, full_path_output_mesh);    
   }

   if(is_off){
       write_off( director, full_path_input_mesh, full_path_output_mesh);
   }
}


int main(){
    translate_mesh("loca_rotada.ply",1,0,0,"loca_rt.ply");
}