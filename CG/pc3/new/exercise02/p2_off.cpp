...
void cube_with_triangular_faces_off( double min_x, double max_x,
double min_y, double max_y,
double min_z, double max_z,
std::string const& full_path_output_file ){

    fstream off_file( full_path_output_file + ".off", ios::out);
    ...
    off_file << "OFF" << endl;
    off_file << cubepoints.size() << " " << faces.size() << " 0" << endl;

    for(auto p:cubepoints){
        p.get_all(off_file);
    }
    for(auto face:faces){
        off_file<<"3 "<<face[0]<<" "<<face[1]<<" "<<face[2]<<endl;
    }

}
int main(){

    cube_with_triangular_faces_off(-1,1,-1,1,-1,1,"cube_t");
