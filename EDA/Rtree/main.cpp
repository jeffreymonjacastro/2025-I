...
bool checkTree(RNode *node, int maxEntries, bool isRoot, int currentDepth,
               int &leafDepth) {
  bool ok = true;
  ...
  if (node->isLeaf) {
    ...
    if (!node->children.empty()) {
      std::cerr << "Error: Nodo hoja no debe tener hijos. Tiene "
                << node->children.size() << " hijos." << std::endl;
      ok = false;
    }
    ...
    if (!isRoot) {
      if (pointCount < (size_t)minCapacity) {
        std::cerr << "Error: Nodo hoja. Cantidad actual de puntos: "
                  << pointCount << ". Se esperaba al menos " << minCapacity
                  << " puntos." << std::endl;
        ok = false;
      }
    }
    if (pointCount > (size_t)maxEntries) {
      std::cerr << "Error: Nodo hoja. Cantidad actual de puntos: " << pointCount
                << ". Se esperaba como maximo " << maxEntries << " puntos."
                << std::endl;
      ok = false;
    }

    // Comprobar que el MBB es mínimo: debe coincidir con el calculado a partir
    // de los puntos.
    MBB computed;
    ...
    if (!mbbEqual(node->mbr, computed)) {
      std::cerr << "Error: MBB minimo inconsistente en nodo hoja. "
                << "Se esperaba " << "{" << computed.lower.x << ","
                << computed.lower.y << "}-{" << computed.upper.x << ","
                << computed.upper.y << "}, "
                << "pero se tiene " << "{" << node->mbr.lower.x << ","
                << node->mbr.lower.y << "}-{" << node->mbr.upper.x << ","
                << node->mbr.upper.y << "}" << std::endl;
      ok = false;
    }
    ...
    if (leafDepth == -1)
      ...
      std::cerr << "Error: Arbol desequilibrado. Una hoja tiene profundidad "
                << currentDepth
                << " y se esperaba que todas las hojas estuvieran en la misma "
                   "profundidad ("
                << leafDepth << ")." << std::endl;
      ok = false;
    }
    ...
    if (!node->points.empty()) {
      std::cerr << "Error: Nodo interno no debe tener puntos. Tiene "
                << node->points.size() << " puntos." << std::endl;
      ok = false;
    }
    ...
    if (!isRoot) {
      if (childCount < (size_t)minCapacity) {
        std::cerr << "Error: Nodo interno. Cantidad actual de hijos: "
                  << childCount << ". Se esperaba al menos " << minCapacity
                  << " hijos." << std::endl;
        ok = false;
      }
    }
    if (childCount > (size_t)maxEntries) {
      std::cerr << "Error: Nodo interno. Cantidad actual de hijos: "
                << childCount << ". Se esperaba como máximo " << maxEntries
                << " hijos." << std::endl;
      ok = false;
    }
    ...
    if (!node->children.empty()) {
      ...
      for (size_t i = 1; i < node->children.size(); i++) {
        computed.lower.x =
            std::min(computed.lower.x, node->children[i]->mbr.lower.x);
        computed.lower.y =
            std::min(computed.lower.y, node->children[i]->mbr.lower.y);
        computed.upper.x =
            std::max(computed.upper.x, node->children[i]->mbr.upper.x);
        computed.upper.y =
            std::max(computed.upper.y, node->children[i]->mbr.upper.y);
      }
    }
    if (!mbbEqual(node->mbr, computed)) {
    ...
    for (RNode *child : node->children) {
      if (!(node->mbr.lower.x <= child->mbr.lower.x &&
        ...
        std::cerr
            << "Error: El MBB de un hijo no está contenido en el nodo interno. "
            << "MBB del padre: {" << node->mbr.lower.x << ","
            << node->mbr.lower.y << "}-{" << node->mbr.upper.x << ","
            << node->mbr.upper.y << "}, "
            << "MBB del hijo: {" << child->mbr.lower.x << ","
            << child->mbr.lower.y << "}-{" << child->mbr.upper.x << ","
            << child->mbr.upper.y << "}" << std::endl;
        ok = false;
      }
...
void testTreeStructure(RTree &tree) {
  ...
  if (checkTree(tree.root, tree.maxEntries, true, 0, leafDepth))
    std::cout << "El árbol cumple con todas las invariantes avanzadas."
              << std::endl;
  else
    std::cerr << "ERROR: Fallaron las invariantes avanzadas del árbol."
              << std::endl;
}
...
void testSearch(RTree &tree, const std::vector<Point> &allPoints) {
  ...
  for (int i = 0; i < numQueries; i++) {
    ...
    auto cmp = [](const Point &a, const Point &b) {
      return a.x < b.x || (approxEqual(a.x, b.x) && a.y < b.y);
    };
    std::sort(result.begin(), result.end(), cmp);
    std::sort(expected.begin(), expected.end(), cmp);
    if (result.size() != expected.size()) {
      std::cerr << "Error en búsqueda: para query (" << query.lower.x << ","
                << query.lower.y << ")-(" << query.upper.x << ","
                << query.upper.y << ") se esperaban " << expected.size()
                << " resultados, pero se obtuvieron " << result.size() << ".\n";
      continue;
    }
    for (size_t j = 0; j < result.size(); j++) {
      if (!approxEqual(result[j].x, expected[j].x) ||
          !approxEqual(result[j].y, expected[j].y)) {
        std::cerr << "Error en búsqueda: los puntos retornados no coinciden "
                     "con los esperados.\n";
        break;
      }
    }
