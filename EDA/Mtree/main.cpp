...
std::string randomString(std::mt19937 &gen, std::size_t len = 9) {
    ...
    std::string s;
    s.reserve(len);
    for (std::size_t i = 0; i < len; ++i) s.push_back(alphabet[d(gen)]);
    return s;
}
void collectObjects(const MNode* node, std::vector<const Object*>& out) {
    if (node->isLeaf()) {
        for (const Object* o : node->objects()) out.push_back(o);
    } else {
...
bool subtreeContains(const MNode* node, const Object* obj) {
    if (node->isLeaf()) {
        return std::find(node->objects().begin(),
                         node->objects().end(), obj) != node->objects().end();
    }
    for (const MNode* child : node->children())
        if (subtreeContains(child, obj)) return true;
    return false;
}
...
bool verifyRegions(const MNode* node) {
    ...
    // Recursión
    for (const MNode* child : node->children())
        if (!verifyRegions(child)) return false;
...
bool testKNN(const MTree& tree,
    ...
    for (int t = 0; t < 5; ++t) {
        ...
        for (std::size_t i = 0; i < k; ++i) brute.insert(all[i].second);
